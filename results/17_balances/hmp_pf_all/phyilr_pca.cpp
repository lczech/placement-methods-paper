...
int main( int argc, char** argv )
{
    ...
    auto meta = dfr.from_file( in_meta );
    LOG_INFO << "meta: " << meta.rows() << " x " << meta.cols();
    auto samples = JplaceReader().from_files( utils::dir_list_files( in_jplace, ".*\\.jplace" ));
    ...
    for( size_t i = 0; i < trees.size(); ++i ) {
        locations[i] = meta[ "Location" ][ trees.name_at(i) ];
        regions[i]   = meta[ "Region" ][ trees.name_at(i) ];
    }
