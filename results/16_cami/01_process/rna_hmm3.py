    ...
    for moltype in options.moltypes.split(','):
        ...
        cmd = 'hmmsearch --cpu %d -o %s --domtblout %s -E %g %s/%s_%s.hmm %s' % \
            (options.p, hmm_out_fname, dom_out_fname, \
                options.evalue,os.path.abspath(options.hmm_path),kingdom,moltype,out_fname+'.fasta')
#        print cmd
...
ff = open(out_fname,"w")
dict_rRNA = {'arc_lsu':'arc 23S_rRNA','arc_ssu':'arc 16S_rRNA','arc_tsu':'arc 5S_rRNA',
             'bac_lsu':'bac 23S_rRNA','bac_ssu':'bac 16S_rRNA','bac_tsu':'bac 5S_rRNA',
             'euk_lsu':'euk 28S_rRNA','euk_ssu':'euk 18S_rRNA','euk_tsu':'euk 8S_rRNA'}
