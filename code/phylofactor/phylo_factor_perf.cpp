...
PFData read_data(
    ...
    auto const samples = JplaceReader().read( from_files( utils::dir_list_files( in_jplace, ".*\\.jplace" )));
    ...
    data.tree_names = trees.names();

    // Copy the meta data in the correct sample order.
    LOG_INFO << "Get meta vector";
    if( sort_col_name.empty() && meta.cols() == 1 ) {
        sort_col_name = meta[0].name();
    }
    if( ! sort_col_name.empty() ) {
        auto meta_vals = std::vector<double>( trees.size(), 0.0 );
        assert( meta.rows() == trees.size() );
        for( size_t i = 0; i < trees.size(); ++i ) {
            meta_vals[i] = meta[ sort_col_name ].as<double>()[ trees.name_at(i) ];
        }
        data.col_sort_indices = sort_indices( meta_vals.begin(), meta_vals.end() );
        data.main_col_idx = meta[ sort_col_name ].index();
    } else {
        data.col_sort_indices = std::vector<size_t>( trees.size() );
        std::iota( data.col_sort_indices.begin(), data.col_sort_indices.end(), 0 );
    }

    data.meta_vals_mat = Matrix<double>( trees.size(), meta.cols() );
    data.col_names = std::vector<std::string>( meta.cols() );
    assert( meta.rows() == trees.size() );
    for( size_t i = 0; i < trees.size(); ++i ) {
        for( size_t c = 0; c < meta.cols(); ++c ) {
            data.meta_vals_mat( i, c ) = meta[ c ].as<double>()[ trees.name_at(i) ];
        }
    }
    for( size_t c = 0; c < meta.cols(); ++c ) {
    ...
    data.bal_data = mass_balance_data( trees, settings );
...
void calc_and_write_balances_mat_and_pca( PFData const& data, std::string const& out_dir )
{
    ...
    utils::MatrixWriter<double>().to_file(
        mat, out_dir + "edge_balances_all.csv", data.tree_names
    );
    ...
    auto proj_meta = utils::Matrix<double>( data.tree_names.size(), components + data.meta_vals_mat.cols(), 0.0 );
    ...
    utils::MatrixWriter<double>().to_file(
        proj_meta, out_dir + "edge_balances_pca_proj_meta.csv", data.tree_names
    );
...
void write_edge_masses_taxon_weights( PFData const& data, std::string const& out_dir )
{
    ...
    utils::MatrixWriter<double>().to_file(
        data.bal_data.edge_masses, out_dir + "edge_masses.csv", data.tree_names
    );
...
void write_factor_edges(
    ...
    assert( data.col_sort_indices.size() == data.tree_names.size() );
    auto balances = utils::Matrix<double>(
        data.tree_names.size(), data.meta_vals_mat.cols() + found_factors
    );
    ...
    utils::MatrixWriter<double>().to_file(
        balances, out_dir + "factor_balances.csv", data.tree_names
    );
}
...
int main( int argc, char** argv )
{
    ...
    utils::Options::get().number_of_threads( 20 );
    ...
    std::string in_meta = "path/to/17_balances/hmp_pf_all/meta_9194_regions_clean.csv";
    std::string in_label_file = "";
    std::string main_col = "";
    std::string out_dir = "path/to/17_balances/hmp_pf_all/";
    ...
    auto const data = read_data( in_jplace, in_meta, settings, in_label_file, main_col, line_width, meta_sep_char );
