...
int main( int argc, char** argv )
{
    ...
    for( auto const& bplace_filename : bplace_filenames ) {
        ...
        sset.add( bplace_loader.load( bplace_filename ), bplace_filename );
        ++file_cnt;
    }
    ...
    LOG_INFO << "Binning masses";
    for( auto& mt : mass_trees.first ) {
        mass_tree_binify_masses( mt, 16 );
    }

    LOG_INFO << "Kmeans started";
    auto mkmeans = tree::MassTreeKmeans();
    mkmeans.accumulate_centroid_masses(16);
